    ...
    if filename.endswith('.dat'):
        ...
        #re-organize arrays so that theres an array for each column not each row
            ...
            if np.max(time_list) >= 90:
                if hex_path <=300:
                    max_times[0] = np.append(max_times[0], np.max(time_list))
                   
                    max_times[1] = np.append(max_times[1],hex(hex_path))
...
print(max_times[0][:20],max_times[1][:20], np.mean(max_times[0]),np.median(max_times[0]), len(max_times[0]))
