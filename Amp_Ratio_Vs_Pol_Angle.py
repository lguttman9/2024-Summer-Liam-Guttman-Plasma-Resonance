...
plots_clean = [plot[valid_mask] for plot in plots]

# Ensure all arrays have the same length
min_length = min(len(x_clean), len(y_clean), len(plots_clean[3]))

x_clean = x_clean[:min_length]
y_clean = y_clean[:min_length]
c_clean = plots_clean[9][:min_length]
...
try:
    ...
    plt.title("log of Max Amplitude Ratio vs Polarization Angle of 100 hand picked overdense Meteors, with SNR greater then 20,  (accounting for gain patterns)")
    plt.legend()
    plt.show()
